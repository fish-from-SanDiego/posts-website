...
function renderComments(jsonData, range) {
  ...
  jsonData
    .filter(
      (comment) =>
        comment.id != null &&
        comment.id >= range.lower &&
        comment.id < range.upper,
    )
    .forEach((comment) => {
      if (comment.body == null || comment.name == null) return;
      const hrElement = document.createElement('hr');
      hrElement.classList.add('comments__hr');
      const commentElement = document.createElement('div');
      commentElement.classList.add('comments__comment');
      commentElement.innerHTML = `<p class="comments_comment-author">${comment.name}</p>
